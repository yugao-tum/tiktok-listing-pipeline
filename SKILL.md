...
5. Inspect every strict-unique image for text and visual duplication. Use `uncertain_requires_review` rather than converting weak OCR into “no text.” Exclude wrong-variant and mixed-model images from the publish sequence.
...
7. Create one ChatGPT project conversation per product. Upload up to five confirmed text-bearing images per request. Preserve composition, product, colors, icons, dimensions, and numbers; replace only approved text. If the UI call times out, observe current state before deciding whether the action failed.
8. Download every generated output, verify it visually, record its hash, and replace the original-language image in the authoritative publish sequence. “Generated” is not “delivered” until the local file and sequence both reference the output.
...
10. Run `scripts/校验产品发布包.py`. A `PASS` advances the product and round to `qa_passed` and binds an artifact hash to that exact snapshot.
...
Use: `queued → leased → evidence_ready → assets_ready → draft_ready → translation_ready → qa_passed → ready_to_commit → committed`.
...
- Stop publication for wrong SKU, excluded asset leakage, missing required image, untranslated source in the publish list, invalid path, corrupt file, or failed writeback comparison.
...
- `scripts/校验产品发布包.py`: enforce local publish gates.
